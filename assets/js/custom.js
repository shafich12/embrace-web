 (function ($) {
        ...
        async function reloadAjax($off) {
            ...
            await $('.marquee').marquee({
                duration: dsnGrid.isMobile() ? 5000 : 9000,
                gap: 30,
                delayBeforeStart: 0,
                direction: 'left',
                duplicated: true
        ...
        function dsnAjax() {
            ...
            return {
                ...
                ajaxNextProject: function ($e) {
                    ...
                    tl.call(this.dsnCreateElement.bind(this, img, active, title, title, {
            ...
            return {
                ...
                nextProject: function () {
                    const _next = $('.next-project'),
                        ...
                        _img = _next.find('.img-footer-parallax'),
                        _gradient = _next.find('.gradient'),
                        tween = gsap.timeline();
                    if (!_next.length) return;


                    if (_img.length)
                        tween.to(_img, {left: 0, width: '100%', height: "100vh"}, 0);

                    if (_gradient.length)
                        tween.to(_gradient, {autoAlpha: 0}, 0);
                    ...
                    parallaxIt.on('progress', function ($s) {
                        ...
                        _next.find('.title-cover .number').text((($s.progress) * 100).toFixed(0) + "%");
                        if ( $s.progress > 0.998 ) {
                            _next.find( 'a' ).click();
                        }
                    ...
                    $('.dsn-header-parallax').each(function () {
                        ...
                        if (heroImg.length) {
                            parallax.to(heroImg, {
                                ...
                                scale: $this.hasClass('has-scale') ? 1.2 : 1
                ...
                parallaxImgHover: function () {
                    ...
                    $("[data-dsn-parallax=\"parallax\"]").each(function () {
                        let _that = $(this);
                        _that.removeAttr("data-dsn-parallax");
                        _that.removeAttr("data-dsn-move");


                        dsnGrid.parallaxMoveElement({
                                target: _that,
                                element: _that.find('.dsn-move-img')
                            },
                            dsnGrid.getData(_that, "move", 20),
                            dsnGrid.getData(_that, "speed", 0.1),
                            _that.find(".dsn-parallax-rev"),
                ...
                dsnScrollTop: function () {
                    ...
                    gsap.to( ".scroll-to-top", 1, { right: -100, visibility:"hidden" } );
        ...
        function Isotope() {
            ...
            $('.dsn-filter').each(function () {
                const $button = $(this).find('.filterings .filtering '),
